...
export async function detectWorkflow(projectRoot, state) {
  ...
  if (reversaRebuild) {
    ...
    for (const [artifact, nextAgent, stage] of rebuildStages) {
      if (!(await exists(join(product, artifact)))) return { stage, nextAgent, reason: `${artifact} ainda não existe` };
    }
  } else {
    const productStages = [
  ...
  if (reversaRebuild) {
    if (!(await exists(join(work, 'parity-report.md')))) {
      return { stage: 'parity', nextAgent: 'factory-reversa-parity', reason: 'parity-report.md ainda não existe' };
    }
    if (!(await approved(join(work, 'parity-report.md')))) {
      return { stage: 'parity-blocked', nextAgent: 'factory-developer', reason: 'paridade ainda não está aprovada' };
    }
    if (!(await exists(join(work, 'cutover-plan.md')))) {
      return { stage: 'cutover', nextAgent: 'factory-reversa-cutover', reason: 'cutover-plan.md ainda não existe' };
    }
  }
