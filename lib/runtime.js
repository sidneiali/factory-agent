...
async function collectContext(projectRoot, state) {
  const candidates = [];
  const activeImport = state.imports?.records?.find((item) => item.id === state.imports.active);
  if (activeImport?.snapshot) {
    const snapshot = safePath(projectRoot, activeImport.snapshot);
    const source = join(snapshot, 'source');
    for (const name of ['domain.md', 'architecture.md', 'inventory.md', 'confidence-report.md', 'gaps.md']) {
      const path = join(source, name);
      if (await exists(path)) candidates.push(path);
    }
    const specs = join(source, 'sdd');
    if (await exists(specs)) candidates.push(...await listFiles(specs));
    for (const name of ['curation.md', 'traceability.md', 'legacy-baseline.md', 'validation-report.md']) {
  ...
  for (const root of roots) candidates.push(...await listFiles(root));
  ...
  for (const path of unique) {
    ...
    total += bytes;
    chunks.push(`--- ${relative(projectRoot, path).split(String.fromCharCode(92)).join('/')}
${content}`);
  }
  return chunks.join('\n\n');
}
