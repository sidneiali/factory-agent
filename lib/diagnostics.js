...
export async function diagnose(projectRoot) {
  ...
  for (const engineId of state.engines) {
    ...
    const installationMarker = engine?.entryFile || (engine?.assetDestination ? `${engine.assetDestination}/index.ts` : null);
    const installed = Boolean(installationMarker) && await exists(safePath(projectRoot, installationMarker));
